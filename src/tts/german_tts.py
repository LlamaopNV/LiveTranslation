...
Local TTS using Coqui TTS for German voice synthesis
"""
...
class GermanTTS:
    """
    German text-to-speech engine using Coqui TTS
    ...
    def synthesize(
        ...
        """
        Synthesize German text to speech
    ...
    def queue_text(self, text: str) -> None:
        ...
        Args:
            text: German text to queue for playback
        """
        if not self.is_playing:
    ...
    def _playback_worker(self) -> None:
        ...
        while not self.stop_playback_event.is_set():
            try:
                ...
                audio = self.synthesize(text)
                if len(audio) > 0:
                    # Save German audio if enabled
                    if self.save_recordings:
                        self._save_german_audio(audio, text)
    ...
    def _save_german_audio(self, audio_data, text):
        """Save German TTS audio to WAV"""
        try:
            ...
            wav_path = f"recordings/german_{timestamp}_{self.recording_counter}.wav"

            # Convert to int16 and save as WAV
            audio_int16 = (audio_data * 32767).astype(np.int16)
            wavfile.write(wav_path, 22050, audio_int16)

            print(f"✓ Saved German audio: {wav_path}")

        except Exception as e:
            print(f"Error saving German audio: {e}")
    ...
    def get_available_models(self) -> List[str]:
        """Get list of available German TTS models"""
        all_models = TTS().list_models()
        german_models = [m for m in all_models if "/de/" in m or "german" in m.lower()]
...
def test_german_tts():
    """Test function for German TTS"""
    print("Testing German TTS...")
